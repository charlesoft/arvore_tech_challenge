...
- Criei o model Entity para representar a Entidade Entity, no qual seria única entidade a representar os diferentes tipos dela mesma que podemos criar, como foi determinado na descrição do exercício.
  - Uma entity por pertencer (belongs_to) our ter muitas (has_many) entidades relacionadas.
  - Adicionei um type `Ecto.Enum` para o atributo `entity_type` e uma lista dos únicos valores que podem ser atribuidos a esse campo. Com isso garantimos que apenas `school, class ou network` seja os unicos dados válidos para esse campo.
  - O parent_id é apenas obrigatório para uma Entity do tipo `class`, mas as outras entidades podem ou não estar relacionadas a outra entidade. Dado isso, adicionei uma constraint na table de entities para garantir que parent_id é obrigatório apenas para entity `class` e no model Entity adicionei validações para garantir
  que o parent_id da entidade seja um parent_id válido, ou seja, uma entity do tipo `class` deve estar relacionada/pertencer a uma entity do tipo `school`. Já uma entity do tipo school não pode estar relacionada/pertencer ao uma entity do tipo `class`.
...
- Utilizei o GraphQL para a criação da API. Com isso todas as requisições são do tipo POST e com isso o usuário/parceiro que utilzia o serviço terá essa flexibilidade de consultar apenas os dados de que precisa da API. Por exemplo, ao fazer uma consulta por uma entidade, em vez de fazer uma query para retornar todos os dados, é possível apenas retornar o `name`.
